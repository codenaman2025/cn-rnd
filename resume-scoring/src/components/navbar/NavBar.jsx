  ...
  const btnData = [
    {
      ...
      type: "Apply",
      style:
        "flex items-center gap-x-2 border border-gray-400 !p-1 !pl-4 !pr-4 rounded-xs cursor-pointer hover:bg-gray-200 transition-colors duration-200",
    ...
    },
    {
      ...
      type: "Dashboard",
      style:
        "flex items-center gap-x-2 border border-white bg-black text-white !p-1 !pl-4 !pr-4 rounded-xs cursor-pointer hover:bg-gray-800 transition-colors duration-200",
      icon: <ImStatsDots />,
    },
  ];
  return (
      ...
      <nav className="w-screen bg-white h-[45px] !p-2 text-black flex justify-evenly items-center border-b-[0.5px] border-gray-500">
        ...
        <div className="">
          <ul className="text-gray-500 flex gap-x-4 text-sm">
            {navItems.map((items) => {
              return (
                <li className="cursor-pointer hover:text-black hover: ">
                  <a href={items.slug}>{items.name}</a>
                </li>
              );
            })}
          </ul>
        ...
        <div className="flex gap-x-3">
          {user?.role && <button onClick={logout}>Logout</button>}
        </div>
      </nav>
    </div>
  );
